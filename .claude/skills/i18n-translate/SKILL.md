...
`draft: true` 的文章不會被 build，所以**草稿階段不需要先翻譯**。草稿還會改，
提前翻譯只會在定稿後全部重做一次。

**但是把 `draft: true` 拿掉（或改成 `false`）之前，必須先補齊四個語系的翻譯檔。**
一旦發布而沒有翻譯，該文章在 `/en` `/id` `/vi` `/th` 底下就不存在，語言切換器會
把讀者退回該語系首頁。

`npm run check:i18n` 對 draft 缺翻譯只會列為警告，對已發布文章缺翻譯則列為錯誤
並以非零狀態結束。發布流程請以這個檢查為準。
